...
void I2c_WriteSub(MyRio_I2c* channel, uint8_t address, uint8_t* data,
               uint32_t numBytes)
{
    const uint8_t timeoutDelay = 1;  /* 1 Second */
    NiFpga_Status status;
    ...
    for (index = 0; (index < numBytes) && (!timeout & !error); index++)
    {
        ...
        if (numBytes == 1)
        {
             ...
             * Generate start bit, send byte, and generate stop bit.
             */
            control = I2c_Start | I2c_Rx_Tx; /* 0b00000111 */
        }
        else
        {
            if (index == 0)
...
void I2c_WriteNoStartBit(MyRio_I2c* channel, uint8_t address, uint8_t* data,
               uint32_t numBytes)
{
    const uint8_t timeoutDelay = 1;  /* 1 Second */
    NiFpga_Status status;
    ...
    for (index = 0; (index < numBytes) && (!timeout & !error); index++)
    {
        ...
        if (numBytes == 1)
        {
             ...
             * Generate start bit, send byte, and generate stop bit.
             */
            control = I2c_Rx_Tx | I2c_Stop; /* 0b00000111 */
        }
        else
        {
            if (index == 0)
...
void I2c_ReadByteFromSub(MyRio_I2c* channel, uint8_t address,uint8_t* subaddress ,uint8_t* data, uint32_t numBytes){
	I2c_WriteSub(channel, address, subaddress,1);
	I2c_Read(channel, address, data, numBytes);
}

void I2c_WriteByteToSub(MyRio_I2c* channel, uint8_t address,uint8_t* subaddress ,uint8_t* data, uint32_t numBytes){
	I2c_WriteSub(channel, address, subaddress,1);
	I2c_WriteNoStartBit(channel, address, data, numBytes);
}

void I2c_ReadByteFromSubLidarLite(MyRio_I2c* channel, uint8_t address,uint8_t* subaddress ,uint8_t* data, uint32_t numBytes){
	I2c_Write(channel, address, subaddress,1);
	I2c_Read(channel, address, data, numBytes);
}
